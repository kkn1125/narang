...
const Typer = (function () {
  function Controller() {
    ...
    // this.disconnect = () => {
    //   window.removeEventListener("load", this.readyToWriting);
    // };
  ...
  function Model() {
      ...
      for (let item in custom) {
        ...
        try {
          ...
          for (let data in custom[item].dataset) {
            let typerData =
              "typer" + data.charAt().toUpperCase() + data.slice(1);
        ...
        if (innerHTML) {
          if (typeof innerHTML == "string") {
            addedWords.push(
              this.getConcat(this.getTyping(this.getSeparate(innerHTML))),
            );
          } else {
            innerHTML.forEach((words) => {
              if (words != "") words = words.trim();
              addedWords.push(
                this.getConcat(this.getTyping(this.getSeparate(words))),
              );
            });
          }
        }
    ...
    this.validTargetByName = function (name) {
      ...
      if (!target) {
        throw new Error(
          "[NoTargetException] 지정한 타겟 중 찾지 못한 타겟이 있습니다.",
        );
      }
      return target;
    };
    ...
    this.writeByOrdered = function () {
      for (let ord in typingBundle) {
        ...
        let startTime = target.target.dataset.typerStart
          ? parseFloat(target.target.dataset.typerStart)
          : initOptions.typer.start;
    ...
    this.createTypingSource = function (item) {
      ...
      if (initOptions.typer.words && initOptions.typer.words[item.name])
        innerHTML = initOptions.typer.words[item.name];
      else if (initOptions.typer.custom && initOptions.typer.custom[item.name])
        innerHTML = initOptions.typer.custom[item.name].words;
      if (innerHTML) {
        if (typeof innerHTML == "string") {
          addedWords.push(
            this.getConcat(this.getTyping(this.getSeparate(innerHTML))),
      ...
      typingBundle.push(
        new this.TypingItem(
          target,
          [this.getConcat(readyToTyping), ...addedWords],
          item,
        ),
      );
    };
    ...
    this.TypingItem = function (target, typing, item) {
      ...
      this.write = function () {
        ...
        let loop = data.typerLoop
          ? data.typerLoop == "false"
            ? false
            : true
          : initOptions.typer.loop;
        let speed =
          data.typerSpeed != undefined
            ? parseFloat(data.typerSpeed)
            : initOptions.typer.speed;
        let delay =
          data.typerDelay != undefined
            ? parseFloat(data.typerDelay)
            : initOptions.typer.delay;
        let loopDelay = data.typerLoopDelay
          ? parseFloat(data.typerLoopDelay)
          : initOptions.typer.loopDelay;
        let eraseMode = data.typerEraseMode
          ? data.typerEraseMode == "false"
            ? false
            : true
          : initOptions.typer.eraseMode;
        let eraseSpeed = data.typerEraseSpeed
          ? parseFloat(data.typerEraseSpeed)
          : initOptions.typer.eraseSpeed;
        let realTyping = data.typerRealTyping
          ? data.typerRealTyping == "false"
            ? false
            : true
          : initOptions.typer.realTyping;
        if (delay < 1) delay = 1;
        if (loopDelay < 1) loopDelay = 1;

        moduleView.doWriting(
          this.target,
          this.typing[this.order],
          speed,
          realTyping,
          isReturn.bind(this),
        );
    ...
    this.validTarget = function () {
      for (let typingItem of typingBundle) {
        ...
        if (target.dataset.typerName.match(/[^rㄱ-힣\w\-]/gm)) {
          throw new Error(
            `[NoTargetException] 지정한 타겟 중 특수문자가 포함되어 있습니다. 특수문자를 제외한 이름으로 지정해주세요. 타겟명: ${target.dataset.typerName}`,
          );
        }
      }
    };
  ...
  function View() {
    ...
    this.doWriting = function (target, data, speed, realTyping, callBack) {
      let initDisplay = requestAnimationFrame(
        this.initializeDisplay.bind(this, target),
      );
      let tmp = "",
        repo = "",
        i = 0,
      ...
      let tps = setInterval(() => {
        ...
        try {
          if (data.length == 0) {
            clearInterval(tps);
            throw new Error(
              `[NoDataException] 내용이 없습니다. 타겟명: ${target.dataset.typerName}`,
            );
          }
        } catch (e) {
          console.error(e.message);
